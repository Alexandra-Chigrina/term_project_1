...
from src.views import (filter_transactions_date, get_card_info, get_top_transactions,
                       get_currency_rates, get_stock_prices, main_page_fnc)
...
def test_filter_transactions_date(transactions_sample_1):
    ...
    final_data = pd.DataFrame(
        [{'Дата операции': '2021-12-05 01:23:42', 'Дата платежа': '05.12.2021', 'Номер карты': '*5091',
         ...
         {'Дата операции': '2021-12-01 22:22:03', 'Дата платежа': '01.12.2021', 'Номер карты': None, 'Статус': 'OK',
          'Сумма операции': -20000.0,
          'Валюта операции': 'RUB', 'Сумма платежа': -20000.0, 'Валюта платежа': 'RUB', 'Кэшбэк': None,
          'Категория': 'Переводы', 'MCC': None, 'Описание': 'Константин Л.', 'Бонусы (включая кэшбэк)': 0,
...
def test_get_card_info(transactions_sample_2):
    expected_result = [{'last_digits': '4556', 'total_spent': 1620.27, 'cashback': 16.2},
...
def test_get_top_transactions(transactions_sample_2):
    expected_result = [{'date': '10.12.2021', 'amount': 20000.0, 'category': 'Другое', 'description': 'Иван С.'},
                       {'date': '01.12.2021', 'amount': 20000.0, 'category': 'Переводы',
                        'description': 'Константин Л.'},
                       {'date': '02.12.2021', 'amount': 1620.27, 'category': 'Супермаркеты',
                        'description': 'Перекрёсток'},
                       {'date': '05.12.2021', 'amount': 564.0, 'category': 'Различные товары',
                        'description': 'Ozon.ru'},
...
@patch("src.views.get_top_transactions", return_value=[{'date': '01.12.2021', 'amount': 20000.0,
                                                        'category': 'Переводы',
                                                        'description': 'Константин Л.'}])
@patch("src.views.get_card_info", return_value=[{'last_digits': '4556',
                                                 'total_spent': 1620.27, 'cashback': 16.2}])
@patch("src.views.filter_transactions_date")
@patch("src.views.read_excel")
@patch("src.views.greeting", return_value='Доброе утро')
def test_main_page_fnc(mock_greeting, mock_read_excel, mock_filter_transactions,
                       mock_get_card_info, mock_get_top_trans,
    ...
    "top_transactions": [{'date': '01.12.2021', 'amount': 20000.0,
                          'category': 'Переводы',
                          'description': 'Константин Л.'}],
